...
export const reportsService = {
  ...
  getPaymentsReport: async (
    ...
    return api.get<PaymentsReport>(`/api/v1/admin/reports/payments?days=${days}`, token);
  },
  ...
  exportData: async (type: string, token: string, format: string = 'csv'): Promise<Blob> => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/api/v1/admin/reports/export/${type}`,
      {
        method: 'POST',
