            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-4 max-w-md">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="currentPassword" name="currentPassword" type={showCurrent ? 'text' : 'password'} value={passwordData.currentPassword} onChange={handlePasswordChange} className="pl-10 pr-10" disabled={isLoading} required />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground pointer-events-none" />
                  <Input id="newPassword" name="newPassword" type={showNew ? 'text' : 'password'} value={passwordData.newPassword} onChange={handlePasswordChange} className="pl-10 pr-10" disabled={isLoading} required />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground pointer-events-none" />
                  <Input id="confirmPassword" name="confirmPassword" type="password" value={passwordData.confirmPassword} onChange={handlePasswordChange} className="pl-10" disabled={isLoading} required />
                </div>
              </div>
              <div className="bg-secondary/50 rounded p-3 text-xs text-muted-foreground">
                ...
                <ul className="list-disc list-inside space-y-0.5">
                  <li>At least 8 characters</li><li>Mix of uppercase and lowercase</li><li>At least one number</li>
                </ul>
              </div>
              {message && (
                <div className={`rounded text-sm p-3 ${message.type === 'success' ? 'bg-green-50 border border-green-200 text-green-700' : 'bg-destructive/10 border border-destructive/20 text-destructive'}`}>
                  {message.text}
                </div>
              )}
          ...
          <div className="bg-card rounded-lg border border-border p-6">
            ...
            <div className="space-y-4 max-w-md">
              {Object.entries(notifications).map(([key, val]) => (
                <label key={key} className="flex items-center gap-3 cursor-pointer p-4 bg-secondary/50 rounded-lg hover:bg-secondary transition-colors">
                  <input type="checkbox" checked={val} onChange={() => setNotifications(prev => ({ ...prev, [key]: !prev[key as keyof typeof prev] }))} className="w-4 h-4 rounded accent-primary" />
                  <div>
